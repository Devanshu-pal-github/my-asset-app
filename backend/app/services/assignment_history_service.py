...
def assign_asset_to_employee(
    ...
    try:
        ...
        if not category["is_reassignable"] and asset["assignment_history"]:
        ...
        if not policies.get("allow_multiple_assignments") and len(employee_ids) > 1:
            logger.warning(f"Multiple assignments not allowed for asset {asset_id}")
            raise ValueError("Multiple assignments not allowed")
        if len(employee_ids) > policies.get("max_assignments", 1):
            logger.warning(f"Exceeds max assignments ({policies.get('max_assignments')}) for asset {asset_id}")
            raise ValueError(f"Cannot assign to more than {policies.get('max_assignments')} employees")
        ...
        update_dict = {
            ...
            "$push": {"assignment_history": assignment_entry.dict()}
        }
        ...
        for emp_id in employee_ids:
            db.employees.update_one(
                {"_id": ObjectId(emp_id)},
                {"$push": {"assigned_assets": {"asset_id": asset_id, "assigned_at": datetime.utcnow()}}}
            )
