...
const AssetInventory = ({
  ...
  const [assetToDelete, setAssetToDelete] = useState(null); // For single asset deletion

  // Calculate summary statistics
  const totalAssets = assets.reduce((sum, asset) => sum + asset.count, 0);
  const activeAssets = assets.reduce(
    (sum, asset) => sum + Math.floor(asset.count * 0.8),
    0
  );
  const totalValue = assets.reduce((sum, asset) => sum + (asset.value || 0), 0);
  const assetsInMaintenance = assets.reduce(
    (sum, asset) => sum + Math.floor(asset.count * 0.1),
    0
  );
  const avgAssetValue =
    totalAssets > 0 ? (totalValue / totalAssets).toFixed(2) : 0;
        ...
        <div className="p-5 border-b border-gray-100 flex justify-between items-start">
          ...
          <div className="flex space-x-2">
            ...
            >
              <i
                className={`pi ${
                  isSelected ? "pi-check-square" : "pi-square"
                } text-lg`}
              ></i>
            </button>
          </div>
    ...
    <div className="py-8 px-10 mt-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border border-gray-100 hover:shadow-lg transition-all duration-200">
          ...
          <div className="flex justify-between items-end">
            <div className="text-3xl font-bold text-gray-900">
              {assetsInMaintenance.toLocaleString()}
            </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border border-gray-100 hover:shadow-lg transition-all duration-200">
          ...
          <div className="flex justify-between items-end">
            <div className="text-3xl font-bold text-gray-900">
              {newAssetsThisMonth.toLocaleString()}
            </div>
      ...
      <FilterBar
        ...
        onAddClick={toggleAddCategoryForm}
      />

      {showAddCategoryForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-[#F5F2EE] rounded-xl p-8 max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl transform transition-all duration-300 ease-in-out">
      ...
      <DeleteConfirmationModal
        ...
        onClose={() => {
          setShowDeleteModal(false);
          setAssetToDelete(null); // Reset single asset deletion
        }}
